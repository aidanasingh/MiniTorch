...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        index_out = np.zeros(len(out_shape), dtype=int)  # [0]*len(out_shape)
        index_in = np.zeros(len(in_shape), dtype=int)  # [0]*len(in_shape)
        # Idea: use broadcast_index, index_to_position, shape_broadcast, to_index
        # iterate through positions of out_storage
        for i in range(len(out)):
            ...
            broadcast_index(index_out, out_shape, in_shape, index_in)  # in is smaller

            # get position in in_storage
            in_pos = index_to_position(index_in, in_strides)
            out_pos = index_to_position(index_out, out_strides)  # new
            # do out_storage[index2] = fn(in_storage[index1])
...
def tensor_zip(fn: Callable[[float, float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        # iterate through positions of out_storage
        index_out = np.zeros(len(out_shape), dtype=int)  # [0]*len(out_shape)
        a_index = np.zeros(len(a_shape), dtype=int)  # [0]*len(a_shape)
        b_index = np.zeros(len(b_shape), dtype=int)  # [0]*len(b_shape)
...
def tensor_reduce(fn: Callable[[float, float], float]) -> Any:
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        # idea: iterate through elements of a_shape[reduce_dim], doing out[..]= fn(out[...], apos)
        # out = [1.0 for i in range(len(out))] ... we might be able to assume out is ones or zeros ?
        iterable_size = a_shape[reduce_dim]
        out_index = np.zeros(len(out_shape), dtype=int)  # [0]*len(out_shape)
